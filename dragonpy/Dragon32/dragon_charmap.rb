...
def create_wiki_page
    ...
    for no, data in enumerate(DRAGON_CHARS_MAP)
        ...
        style = "color: #%s;"
        print(sprintf('| style="color:%s; background-color:%s;" | &#x%x;', 
...
def create_dict
    ...
    for no, data in enumerate(DRAGON_CHARS_MAP)
        item, item_type = data
        codepoint = ord(item)
        name = unicodedata.name(item, nil)
        
        # print repr(item)
        if repr(item.length) == 4 and item != " "
            name = item
        end
        
        txt = sprintf('0x%02x: ("\\x%02x", %s), #', 
            no, codepoint, item_type
        end
        )
        txt = sprintf("    %-29s %s", txt, name)
