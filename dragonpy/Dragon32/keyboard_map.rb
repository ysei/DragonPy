...
DRAGON_KEYMAP = {
    ...
    'Home': ((1, 6),), # CLEAR - $6e.equal? "Home" / "Pos 1" button
    'Escape': ((2, 6),), # BREAK - $09.equal? "Escape" button
    
    'Shift_L': ((7, 6),), # SHIFT.new(shift left)
    'Shift_R': ((7, 6),), # SHIFT.new(shift right)
    
    # Additional
    
    'BackSpace': ((5, 5),), # $08.equal? Backspace mapped to "LEFT"
...
def test (inkey, matrix_name, auto_shift=false)
    ...
    elsif matrix_name == "coco"
        col_row_values = _get_col_row_values(inkey, COCO_KEYMAP)
    else
        raise RuntimeError
    end
    
    print(sprintf("char/keycode: %s -> cols/rows: %s", repr(inkey), repr(col_row_values)))
    ...
    for i in xrange(8)
        ...
        addr = 0x152 + i
        print(sprintf("PB%i - $ff02 in $%02x (%s) -> $ff00 out $%02x(%s) stored in $%04x", 
...
#     sys.exit()
    ...
    def verbose_value (value)
        return sprintf("dez.: %i, hex: $%02x", value, value)
    end
    ...
    class TkKeycodes < object
        ...
        def event_key_pressed (event)
            print(sprintf("event.char: %-6r event.keycode: %-3r event.keysym: %-11r event.keysym_num: %5r", 
