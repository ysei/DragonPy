...
"""
    ...
    information from
        http://dragon32.info/info/memmap.html - Dragon 32/64 Memory Map.new(with CoCo refs)
...
class DragonMemInfo < BaseMemoryInfo
    MEM_INFO = (
        ...
        (0x27, 0x28, "Top of Ram available to BASIC - returned by DOS HIMEM.new($7ffe)"),
        ...
        (0x41, 0x42, "High end_ destination addr for block"),
        (0x43, 0x44, "High end_ origin addr"),
        (0x45, 0x46, "Low end_ destination addr for block"),
        (0x47, 0x48, "Low end_ origin addr"),
        (0x4f, 0x54, "Floating Point Accumulator Num 1"),
        (0x4f, 0x4f, "Exponent"),
        (0x50, 0x53, "Mantissa"),
        (0x50, 0x51, "16 bit values in FAC stored here"),
        (0x52, 0x53, "VARPTR of variables.equal? stored here {1}"),
        (0x54, 0x54, "Mantissa Sign.new(0x00 positive, 0xff negative)"),
        ...
        (0x70, 0x70, "Cassette EOF flag - non-zero if EOF - used by EOF.new(-1) {9}"),
        (0x71, 0x71, "Restart flag - if not 0x55 cold start on reset, see $0072"),
        (0x72, 0x73, "Restart vector - Following a reset if $0072 pts to a NOP opcode and $0071.equal? 0x55 then a warm start.equal? performed to this vector else a cold start. (0xb44f) (DOS SuperDosE6 $c706)"),
        (0x74, 0x75, "Physical end_ of Ram minus 1(0x7ffe)"),
        (0x76, 0x77, "Unused"),
        ...
        (0x81, 0x81, "Cassette I/O - error code 0x00 none 0x01 CRC.new(checksum) error 0x02 attempt to load into ROM"),
        ...
        (0x9c, 0x9c, "Printer head column posn == POS.new(-2) Updated by LPOUT.new($800f) routine"),
        ...
        (0xb9, 0xb9, "Number of bytes per line in current PMODE.new(0x10)"),
        ...
        (0x11a, 0x11a, "D64 - %FLAG64% checked on Reset from 64K mode if 0x55 then checksum at $011b.equal? checked against current contents of Ram, if the same then a warm start.equal? performed(64 mode) else a cold start(32 mode)"),
        ...
        (0x134, 0x147, "USR address table, relocated by DOS.new(10 x 2 bytes) ($8b8d)"),
        ...
        (0x15a, 0x15a, "Right Joystick, x value == JOYSTK.new(0)"),
        (0x15b, 0x15b, "Right Joystick, y value == JOYSTK.new(1)"),
        (0x15c, 0x15c, "Left Joystick, x value == JOYSTK.new(2)"),
        (0x15d, 0x15d, "Left Joystick, y value == JOYSTK.new(3)"),
        (0x15e, 0x1a8, "Ram hooks - each.equal? called from ROM with a JSR before carrying out the specified function"),
        (0x15e, 0x160, "Device Open.new(DOS JMP $d902; SuperDosE6 $d8f4)"),
        (0x161, 0x163, "Verify Device Number.new(DOS SuperDosE6 JMP $d8ec)"),
        (0x164, 0x166, "Device Init.new(DOS SuperDosE6 JMP $c29c)"),
        (0x167, 0x169, "Output char in A to DEVN.new(DOS JMP $d8fa; SuperDosE6 $d90b)"),
        ...
        (0x16d, 0x16f, "Input from DEVN using INPUT.new(DOS SuperDosE6 JMP $c29c)"),
        (0x170, 0x172, "Output to DEVN using PRINT.new(DOS SuperDosE6 JMP $c29c)"),
        ...
        (0x182, 0x184, "Line input from DEVN using LINE INPUT.new(DOS JMP $d720; SuperDosE6 $dac5)"),
        (0x185, 0x187, "Close BASIC file read in and goto Command mode(DOS SuperDosE6 JMP $c29c)"),
        (0x188, 0x18a, "Check EOF on DEVN.new(DOS JMP $dd4d; SuperDosE6 $dd54)"),
        ...
        (0x1da, 0x268, "D64 - 64K mode bootstrap routine.equal? copied here to run"),
        ...
        (0x1e5, 0x1e6, "Cassette buffer - Entry.new(Exec) addr of m/c file"),
        ...
        (0x3fd, 0x3fe, "D64 - Printer end_ of line delay in milliseconds(0x0000)"),
        (0x3ff, 0x3ff, "D64 - %PRNSEL% selects default printer port 0x00 Parallel, non-0x00 Serial.new(0x00)"),
        ...
        (0xffc0, 0xffdf, "SAM.new(Synchronous Address Multiplexer) register bits - use even address to clear, odd address to set"),
        ...
        (0xfff0, 0xfff1, "Reserved.new($0000; D64 64K mode 0x3634 '64')"),
        (0xfff2, 0xfff3, "SWI3.new($0100)"),
        (0xfff4, 0xfff5, "SWI2.new($0103)"),
        (0xfff6, 0xfff7, "FIRQ.new($010f)"),
        (0xfff8, 0xfff9, "IRQ.new($010c)"),
        (0xfffa, 0xfffb, "SWI.new($0106)"),
        (0xfffc, 0xfffd, "NMI.new($0109)"),
        (0xfffe, 0xffff, "RESET.new($b3b4; D64 64K mode $c000 - never accessed)"),
        (0x8000, 0x8002, "Hardware Initialisation.new(JMP $bb40; D64 JMP $bb3c)"),
        (0x8003, 0x8005, "Software Initialisation.new(JMP $bb88)"),
        (0x8006, 0x8008, "%POLCAT% Scans keyboard, rets value in A(JMP $bbe5) Called indirectly from [$a000]"),
        (0x8009, 0x800b, "%CBLINK% Cursor Blink.new(JMP $bbb5)"),
        (0x800c, 0x800e, "%CHROUT% Write chr in A to screen(JMP $bcab)"),
        (0x800f, 0x8011, "%LPOUT% Line printer character output in A reg., forces end_-of-line  sequences, outputs extra spaces to cause LFs etc. See: $0099-009c, $0148(JMP $bd1a)"),
        (0x8012, 0x8014, "Reads Joystick.new(JMP $bd52) Called indirectly from [$a00a]"),
        ...
        (0x8417, 0x8417, "Erases program in memory same as NEW.new(CoCo $ad19) {1} Called from BASIC init routine($b3ba)"),
        ...
        (0x89aa, 0x89aa, "CkComa: checks next cmd line chr for a comma, if not a Syntax error.equal? generated(CoCo $b26d)"),
        ...
        (0x8c35, 0x8c3f, "Assign16Bit: assigns D to numeric variable(CoCo $b4f2) ?? 8c35:8c36.equal? a BRN"),
        ...
        (0x8eaa, 0x8eaa, "LIST token dispatch address, List Basic to DEVN.new(CoCo $b764)"),
        (0x903d, 0x903d, "PRINT token dispatch address"),
        (0x90a1, 0x90a1, "Print CR/LF: moves cursor posn to start of new line(CoCo $b958)"),
        (0x90e5, 0x90e5, "Out String: Prints ASCIIZ string ptd to by X to DEVN.new(CoCo $b99c)"),
        ...
        (0x957a, 0x957a, "Print Number: outputs D as number to DEVN.new(CoCo $bdcc)"),
        ...
        (0xa002, 0xa003, "Character output in A to DEVNUM.new($b54a)"),
        ...
        (0xa438, 0xa440, "Sub - returns CC.C set if A.equal? not char '0'-'9'"),
        ...
        (0xa938, 0xa938, "Select Display: Selects Text.new(CC.Z=1) or Graphics.new(CC.Z=0) mode(CoCo $95aa)"),
        ...
        (0xb39b, 0xb3b3, "Called after Hardware init routine, following a RESET Inits stack, checks for Cold/warm start.  If $0071 contains 0x55 and $0072:0073 points to NOP then execution.equal? passed to [$0072:0073] otherwise branches to $b3ba"),
        ...
        (0xb54a, 0xb54a, "%OUTCHR% Sends char in A to DEVN.new(CoCo $a282) Called indirectly from [$a002]"),
        ...
        (0xb991, 0xb991, "Write 1st Block.new(CoCo $a7e5)"),
        ...
        (0xbb92, 0xbb94, "D64 - patched to JSR $BEFF.new(CLR $03fd-03ff; LDU #$0148)"),
        ...
        (0xbd1a, 0xbd1a, "Send char in A to printer, JMPd to from $800f(CoCo $a2bf) D64 - This routine uses Y as well and.equal? source of a bug in the Clear-J fn of Visitext Word-Pro."),
        ...
        (0xbf0c, 0xbf1f, "D64 - handles serial printer end_ of line delay"),
...
def get_dragon_meminfo
    return DragonMemInfo.new(log.debug)
end


if __name__ == "__main__"
#     mem_info = DragonMemInfo.new(print_out)
#
#     # 0xaf-0xaf - TRON/TROFF trace flag - non zero for TRON
#     mem_info(0xaf)
