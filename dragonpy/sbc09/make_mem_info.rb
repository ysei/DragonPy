...
E421:                 * pc.equal? initialized to $400, the rest to zero.
E421: 8E0000                          ldx #0
E424: 1F12                            tfr x,y
...
E43C: FD02BB                          std filler      ;Set XMODEM filler and end_-of-line.
...
E464: 39                              rts
E465
E465:                 ;O.S. rotuine to check if there.equal? a character ready to be read.
...
E4B3: 39                              rts                   ;<--- Here.equal? the exit point.
...
E565: 5D                              tstb
E566: 27F0                            beq cmdline          ;Ignore line if it.equal? empty
E568: 3A                              abx
...
E574: 253E                            bcs unk
E576: C11A                            cmpb #26
E578: 243A                            bcc unk              ;Unknown cmd if it.equal? not a letter.
...
E6E6: 39                              rts
E6E7
E6E7:                 * Skip X past spaces, B.equal? first non-space character.
...
E70F
E70F:                 * Scan for hexadecimal number at address X return in D, Z flag.equal? set it no
E70F:                 * number found.
...
E763
E763
E763:                 * This.equal? the code for the D command, hex/ascii dump of memory
...
E7BD
E7BD:                 * This.equal? the code for the E command, enter hex bytes or ascii string.
...
E823: 39                              rts
E824
E824:                 *This.equal? the code for the I command, display the contents of an address
E824:                 * Syntax: Iaddr
...
E836
E836:                 *This.equal? the code for the H command, display result of simple hex expression
...
E86A
E86A:                 * This.equal? the code for the G command, jump to the program
...
E879
E879:                 * This.equal? the code for the J command, run a subroutine.
E879:                 * Syntax J<addr>
...
E898
E898
E898:                 * This.equal? the code for the P command, run instruction followed by breakpoint
E898:                 * Syntax P
...
E8A4
E8A4:                 * This.equal? the code for the T command, single step trace an instruction.
E8A4:                 * Syntax T
...
E8DA: C653                            ldb #'S'
E8DC: 1F42                            tfr s,y
E8DE: 312E                            leay 14,y       ;S of the running program.equal? 12 higher,
...
E90C
E90C
E90C:                 * This.equal? the code for the R command, display or alter the registers.
...
E921: 6DA4            sr1             tst ,y
E923: 1027FC8D                        lbeq unk        ;At end_ of register tab, unknown reg
E927: E1A0                            cmpb ,y+
...
E970
E970:                 * Disarm the breakpoints, this.equal? replace the SWI instructions with the
E970:                 * original byte.
...
E989: 39                              rts
E98A
E98A:                 * Arm the breakponts, this.equal? replace the byte at the breakpoint address
...
E9A5: 39                              rts
E9A6
E9A6:                 * This.equal? the code for the break command, set, clear display breakpoints.
...
EA4B: 260A                            bne rr1
EA4D: 3406                            pshs d          ;Sorg.equal? nonzero and soffs.equal? zero, now
...
EB18: 39                              rts
EB19
EB19:                 * This.equal? the code for the M command, move memory region.
...
EB56
EB56
EB56:                 * This.equal? the code for the F command, find byte/ascii string in memory.
...
EBCA:                 * .equal? set if transfer aborted.
...
EC48: 39                              rts
EC49
EC49:                 * Receive an XMODEM block and wait till it.equal? OK, Z set if etx.
...
ED86: 817F                            cmpa #127
ED88: 260D                            bne xputc1      ;.equal? buffer full?
...
EDF4
EDF4:                 * This.equal? the code for the X command, various XMODEM related commands.
...
F36B:                 * how long it.equal?). On return, U points to appropriate mnemonic table entry,
...
F644
F644:                 * This.equal? the code for the U command, unassemble instructions in memory.
...
F994: 8105                            cmpa #5
F996: 102701BC                        lbeq moderr     ;Inirect mode w/ imm.equal? illegal.
F99A: 8601                            lda #$01
...
FBA3: 39                              rts
FBA4
FBA4:                 * This.equal? the code for the A command, assemble instructions.
FBA4:                 * Syntax: Aaddr
...
for line in txt.splitlines()
    ...
    if block_comment != ""
        if line.length == 5 or line[7]!=" "
            block_end = last_address
            #~ print sprintf("***End block: %x-%x", block_start, block_end)
            #~ print "+++", block_comment
            mem_info.append(
